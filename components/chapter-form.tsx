...
export default function ChapterForm({
      ...
      try {
        ...
        if (token) {
          ...
          if (profileRes.ok) {
            ...
            if (profileData?.id) setAuthorId(profileData.id);
          }
        }
        ...
        // 3) Si estamos en modo edición, cargar los datos del capítulo
        if (mode === "edit" && chapterId) {
          const base = process.env.NEXT_PUBLIC_BASE_URL;

          // → Si viene editionId, uso la ruta anidada;
          //   si no, llamo al endpoint de capítulos "propios"
          const chapterUrl = editionId
            ? `${base}/editions/${editionId}/books/${bookId}/chapters/${chapterId}`
            : `${base}/chapters/${chapterId}`;

          const chapterRes = await fetch(chapterUrl);
          if (!chapterRes.ok)
            throw new Error("No se pudo cargar el capítulo para edición");

          const chapterData = await chapterRes.json();

          // 4) Rellenar formulario
          setTitle(chapterData.title ?? "");
          setStudyType(chapterData.studyType ?? "");
          setIntroduction(chapterData.introduction ?? "");
          setObjectives(chapterData.objectives ?? "");
          setMethodology(chapterData.methodology ?? "");
          setResults(chapterData.results ?? "");
          setDiscussion(chapterData.discussion ?? "");
          setBibliography(chapterData.bibliography ?? "");

          // 5) Si ya hay título y tipo, vamos directo al form detallado
          if (chapterData.title && chapterData.studyType) {
            setFormPhase("detailed");
          }
        }
      } catch (err: any) {
  ...
  const renderStepContent = () => {
        ...
        <div className='space-y-6 bg-card p-6 rounded-lg border'>
          ...
          {(!introduction.trim() ||
            ...
            <Alert variant='destructive' className='mt-4'>
              ...
              <AlertDescription>
                Para poder {mode === "edit" ? "guardar" : "enviar"} el capítulo,
                debes completar todas las secciones obligatorias.
    ...
    // Renderizado de cada paso específico
    switch (step) {
      case "Introducción":
        return (
          <>
  ...
  // Estilos para el modo de concentración
  useEffect(() => {
    if (focusMode) {
  ...
  // Función para manejar el envío/guardado
  const handleSubmit = async () => {
    ...
    try {
      ...
      const body = {
        ...
        status: mode === "edit" ? undefined : "pendiente",
        // solo incluyo editionId si existe
        ...(editionId && { editionId }),
      };

      // Elegir URL y método según modo y si es edición o personalizado
      let url: string;
      let method: "POST" | "PUT";
      ...
      if (mode === "edit") {
        ...
        method = "POST";
        url = editionId
          ? `${base}/editions/${editionId}/books/${bookId}/chapters`
      ...
      if (!res.ok) {
        ...
        throw new Error(
          data.message ||
            `Error al ${mode === "edit" ? "actualizar" : "crear"} el capítulo`
        );
      }

      setSuccess(true);

      // Callback o navegación por defecto tras éxito
      if (onSuccess) {
        onSuccess();
      } else {
        setTimeout(() => {
          if (editionId) {
            ...
            router.push(`/publications/chapters`);
          }
        }, 2000);
      }
    } catch (err: any) {
      ...
      <div className='flex items-center justify-center min-h-[60vh]'>
        <motion.div
          ...
          <h2 className='text-2xl font-bold text-foreground mb-2'>
            ¡Capítulo {mode === "edit" ? "actualizado" : "enviado"} con éxito!
          </h2>
          <p className='text-muted-foreground mb-6'>
            Tu capítulo ha sido {mode === "edit" ? "actualizado" : "enviado"}{" "}
            correctamente.
          </p>
          <Button
            ...
            {editionId ? "Ir a la edición" : "Ver mis capítulos"}
          </Button>
        </motion.div>
      </div>
    );
  ...
  // Verificar créditos solo para modo creación y si hay editionId
  if (mode === "create" && editionId && (loadingCredits || loading)) {
    return (
      <div className='flex items-center justify-center min-h-[60vh]'>
        <div className='relative'>
          <div className='h-16 w-16 rounded-full border-t-4 border-b-4 border-primary animate-spin'></div>
      ...
      <div className='container mx-auto px-4 relative z-10'>
        <motion.div
          ...
          <div className='inline-block text-sm font-medium py-1 px-3 rounded-full bg-primary/10 text-primary'>
            {mode === "edit" ? "Editar Capítulo" : "Nuevo Capítulo"}
          </div>
        </motion.div>
          ...
          <Card className='max-w-3xl mx-auto backdrop-blur-sm bg-white/80 border-white/50'>
            <CardHeader>
              <CardTitle className='text-2xl text-primary'>
                {mode === "edit" ? "Editar capítulo" : "Crear nuevo capítulo"}
              </CardTitle>
              <CardDescription>
                {mode === "edit"
                  ? "Modifica la información de tu capítulo"
                  : `Completa la información básica para comenzar a crear tu capítulo para ${bookTitle}`}
              </CardDescription>
            </CardHeader>
